(function($){
	...
	function getRuleIndex(target, field){
         ...
         * @param {type} target
         * @param {type} field
         * @returns {datagrid-filterL#1.getFilterRule.rules}
         */
	...
	function doFilter(target){
		...
		if (opts.remoteFilter){
			...
			if (opts.view.type === 'scrollview' && state.data.firstRows && state.data.firstRows.length){
			 ...
			 var popts = $(target)[name]('getPager').pagination('options');
                         var pn    = popts.pageNumber;
                         var total = popts.total;
