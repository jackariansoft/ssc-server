...
import mude.srl.ssc.modbus.util.ModbusUtil;

import java.io.FilterOutputStream;

/**
 * Class implementing a specialized <tt>OutputStream</tt> which encodes bytes
 * written to the stream into two hexadecimal characters each. Note that the
 * "virtual" characters FRAME_START and FRAME_END are exceptions, they are
 * translated to the respective characters as given by the specification.
 * 
 * @author Dieter Wimberger
 * @version @version@ (@date@)
 * 
 * @see mude.srl.ssc.modbus.io.ModbusASCIITransport#FRAME_START
 * @see mude.srl.ssc.modbus.io.ModbusASCIITransport#FRAME_END
 */
public class ASCIIOutputStream extends FilterOutputStream {

	/**
	 * Constructs a new <tt>ASCIIOutputStream</tt> instance writing to the given
	 * <tt>OutputStream</tt>.
	 * 
	 * @param out
	 *            a base output stream instance to be wrapped.
	 */
	...
	/**
	 * Writes an array of bytes encoded as two hexadecimal characters to the raw
	 * output stream.
	 * 
	 * @param data
	 *            the <tt>byte[]</tt> to be written.
	 * @throws IOException
	 *             if an I/O error occurs.
	 */
