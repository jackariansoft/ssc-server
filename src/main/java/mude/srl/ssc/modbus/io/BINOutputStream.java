 ...
 * written to the stream that resemble a frame token.
 * 
 * Note that the "virtual" characters FRAME_START and FRAME_END are exceptions,
 * they are translated to the respective tokens as given by the specification.
 * 
...
public class BINOutputStream extends FilterOutputStream {
	...
	/**
	 * Writes a byte to the raw output stream. Bytes resembling a frame token
	 * will be duplicated.
	 * 
	 * @param b
	...
	public void write(int b) throws IOException {
		if (b == ModbusASCIITransport.FRAME_START) {
			...
			return;
		} else if (b == ModbusBINTransport.FRAME_START_TOKEN
				|| b == ModbusBINTransport.FRAME_END_TOKEN) {
			out.write(b);
			out.write(b);
		}
	}// write
	...
	/**
	 * Writes an array of bytes to the raw output stream. Bytes resembling a
	 * frame token will be duplicated.
	 * 
	 * @param data
